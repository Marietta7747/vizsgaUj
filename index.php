    ...
    <script>
        ...
        function initMap() {
            const map = new google.maps.Map(document.getElementById("map"), {
                center: { lat: 46.360, lng: 17.728 }, // Replace with KAPOSVAR_TRANSIT_CONFIG.center if needed
                zoom: 13,
                styles: [
                    {
...
<script>
    ...
    try {
        ...
        // Ellenőrizzük, hogy a válasz helyes JSON-e
        if (!response.ok) {
            throw new Error(`Hiba a fájl betöltésekor: ${response.status} ${response.statusText}`);
        }
        else
    {
        ...
        transitData = data;
        console.log('GTFS Data:', transitData);
        addStationMarkers();
    } catch (error) {
        console.error('Hiba a GTFS adatok betöltésekor:', error);
        alert('Nem sikerült betölteni a közlekedési adatokat!');
    }
...
.then(data => {
    console.log(data); // Tovább dolgozhatsz az adatokkal
})
.catch(error => {
...
<script>
    async function loadAndProcessGTFSData(filePath) {
    try {
        const response = await fetch(filePath);
        let text = await response.text();
       
        // JSON szöveg előkészítése parse-oláshoz
        text = text
            // Minden NaN és "NaN" értéket cseréljünk "null"-ra
            .replace(/:\s*(NaN|"NaN")/g, ': null') // NaN és "NaN" kezelés
            .replace(/: NaN,/g, ': null,')        // NaN utáni vesszők kezelése
            .replace(/: NaN\n/g, ': null\n')      // NaN utáni sorvége kezelés
            .replace(/: NaN}/g, ': null}')        // NaN utáni zárójel kezelés
            .replace(/:\s+"NaN"/g, ': null');     // String "NaN" kezelése

        // Debug: Nézzük meg az első néhány sort a feldolgozás után
        console.log("Előfeldolgozott JSON minta:", text.substring(0, 500));
        ...
        try {
            ...
            for (const [key, value] of Object.entries(data)) {
                if (Array.isArray(value)) {
                    processedData[key] = value.map(item => cleanValues(item));
                    console.log(`Feldolgozva: ${key}, ${value.length} rekord`);
                }
            }
            ...
            // Ha a parse hibát dob, próbáljuk meg lokalizálni a hibát
            console.error('JSON parse hiba:', parseError);
           
            // Keressük meg a problémás részt
            const errorPosition = findErrorPosition(text, parseError);
            console.log('Problémás rész környéke:', text.substring(
...
// Értékek tisztítása objektumokban
function cleanValues(item) {
...
// Hibapozíció megkeresése a JSON-ben
...
async function initializeData() {
    try {
        console.log("GTFS adatok betöltése kezdődik...");
       
        // Első próbálkozás az adatok betöltésére
        const data = await loadAndProcessGTFSData('gtfs_data.json');
        console.log("Adatok sikeresen betöltve!");
       
        // Ellenőrzés: első rekordok minden típusból
        for (const [key, value] of Object.entries(data)) {
            if (Array.isArray(value) && value.length > 0) {
                console.log(`${key} első rekord minta:`, value[0]);
            }
        }
       
        return data;
    } catch (error) {
        console.error("Végzetes hiba az adatok betöltésekor:", error);
        throw error;
    }
}
...
initializeData().then(data => {
    // Itt már használhatod az adatokat
    console.log("Betöltés sikeres, adatok feldolgozásra készek");
}).catch(error => {
    console.error("Hiba:", error);
   
    // Adjunk részletes hibajelentést
    console.log("Kérjük, ellenőrizze a JSON fájl formátumát és a NaN értékek kezelését");
});
...
// GTFS adatok betöltése és megjelenítése
// GTFS adatok betöltése és megjelenítése
async function initializeGTFSMap(map, gtfsData) {
    clearMapElements();
   
    // Megállók megjelenítése
    if (gtfsData['stops.txt']) {
        await displayStops(map, gtfsData['stops.txt']);
    }
   
    // Shape-ek megjelenítése
    if (gtfsData['shapes.txt']) {
        await displayShapes(map, gtfsData['shapes.txt']);
    }
   
    // Járatok adatainak feldolgozása
    if (gtfsData['routes.txt']) {
        processRoutes(gtfsData['routes.txt']);
    }
}

// A processRoutes függvény definíciója
function processRoutes(routesData) {
    // Itt dolgozhatod fel az útvonalak adatokat
    // Az alábbi példa kiírja az összes útvonalat a konzolra
    console.log('Elérhető útvonalak:', routesData);
   
    routesData.forEach(route => {
        console.log(`Útvonal neve: ${route.route_name}, Útvonal ID: ${route.route_id}`);
    });
}


// Megállók megjelenítése
async function displayStops(map, stops) {
    ...
    icon: getScaledIcon(map)  // Dinamikusan generált ikon a zoom szint alapján
});

// Térkép zoom szintjéhez igazított ikont biztosító függvény
function getScaledIcon(map) {
    const zoomLevel = map.getZoom();  // Lekérdezi a térkép aktuális zoom szintjét
   
    // A zoom szint függvényében méretezzük az ikont
    const iconSize = Math.max(30, 30 - (zoomLevel - 40));  // Ezt testre szabhatod a kívánt skálázásra
   
    return {
        url: 'marker.png',  // Az ikon URL-je
        scaledSize: new google.maps.Size(iconSize, iconSize + 10),  // Az ikon méretének dinamikus beállítása
        origin: new google.maps.Point(0, 0),  // Az ikon eredeti pozíciója
        anchor: new google.maps.Point(iconSize / 2, iconSize / 2)  // Az ikon középpontja
    };
}
            ...
            stationMarkers.set(stop.stop_id, marker);
...
// Shape-ek (útvonalak) megjelenítése
async function displayShapes(map, shapes) {
    const shapeGroups = new Map();
   
    // Shape pontok csoportosítása shape_id szerint
    ...
    // Shape-ek megjelenítése a térképen
    shapeGroups.forEach((points, shapeId) => {
        // Pontok rendezése sequence szerint
        ...
        polyline.setMap(map);
        routePolylines.set(shapeId, polyline);
...
// Járat kiválasztása és megjelenítése
function showRoute(routeId, gtfsData) {
    const route = gtfsData['routes.txt'].find(r => r.route_id === routeId);
    if (!route) return;
   
    // Kapcsolódó trip-ek keresése
    const trips = gtfsData['trips.txt'].filter(t => t.route_id === routeId);
    if (trips.length === 0) return;
   
    // Shape megjelenítése
    trips.forEach(trip => {
        const polyline = routePolylines.get(trip.shape_id);
        if (polyline) {
            polyline.setOptions({
                strokeColor: COLORS.bus,
                strokeOpacity: 1.0,
...
function initMap() {
    ...
    // GTFS adatok betöltése és megjelenítése
    loadAndProcessGTFSData('gtfs_data.json')
        .then(data => {
            initializeGTFSMap(map, data);
           
            // Eseménykezelők hozzáadása a tranzit mód gombokhoz
            document.querySelectorAll('.transit-mode-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    const mode = btn.dataset.mode;
                    updateMapForMode(mode, data);
                });
            });
        })
...
// Térkép frissítése a választott közlekedési mód szerint
function updateMapForMode(mode, gtfsData) {
    clearMapElements();
   
    let routes;
    switch(mode) {
        case 'bus':
            routes = gtfsData['routes.txt'].filter(r => r.route_type === '3');
            break;
        case 'train':
            routes = gtfsData['routes.txt'].filter(r => r.route_type === '2');
            break;
        case 'complex':
            routes = gtfsData['routes.txt'];
            break;
    }
   
    if (routes) {
        routes.forEach(route => showRoute(route.route_id, gtfsData));
    }
}
